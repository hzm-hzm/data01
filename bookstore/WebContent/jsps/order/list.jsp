					...
					<td colspan="7" style="text-align: left;">
						...
						<c:choose>
							<c:when test="${order.state eq 1 }">
								<a href="<c:url value='/OrdersServlet?method=findorderbyoid&oid=${order.oid }'/>">付款</a>
							</c:when>
							<c:when test="${order.state eq 2 }">等待发货</c:when>
							<c:when test="${order.state eq 3 }">
								<a href="<c:url value='/OrdersServlet?method=changestatebyoid&oid=${order.oid }'/>">确认收货</a>
							</c:when>
							<c:when test="${order.state eq 4 }">
								<a href="<c:url value='/OrdersServlet?method=delectbyoid&oid=${order.oid }'/>"">删除记录</a>
							</c:when>	
						</c:choose>
				</tr>
